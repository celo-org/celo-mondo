...
export function HomePage({ initialData }: { initialData?: string }) {
  // Serialized because RSC prop serialization downgrades bigints to strings
  const initialGroups = useMemo(
    () => (initialData ? deserializeBigints<ValidatorGroupInfo>(initialData) : undefined),
    [initialData],
  );
  const { groups, totalVotes } = useValidatorGroups(false, initialGroups);
