...
import { Address, decodeFunctionData, formatUnits, isAddress } from 'viem';
...
async function decodeWithCommonSelectors(
  ...
  let selectorInfo = (await commonSelectors)[selector];
  if (!selectorInfo) {
    const sig: {
      count: number;
      next: number;
    ...
    if (sig.count > 0) {
      commonSelectors[selector] = { name: sig.results.at(-1)!.text_signature };
    }
    selectorInfo = commonSelectors[selector];
  }
...
/**
 * Generates human-readable descriptions for governance functions
 */
function getTransactionDescription(functionName: string, args: any[]): string {
...
/**
 * Gets a human-readable contract name from an address
 */
export function getContractName(address: Address): string {
  if (!address) {
    return 'Unknown Contract';
  }
  // Common Celo contract addresses
...
/**
 * Formats transaction arguments for display
 */
export function formatTransactionArgs(args: Record<string, any>): string {
