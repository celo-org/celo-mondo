...
// NOTE: hydrate from localstorage -- note the file runs on server too (and in
// test environments where localStorage may exist but lack a getItem method).
const cachedNames =
  typeof globalThis.localStorage?.getItem === 'function'
    ? globalThis.localStorage.getItem(CACHE_KEY)
    : null;
const singleton: ENSMap = JSON.parse(cachedNames || '{}');
