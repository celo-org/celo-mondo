...
describe('AmountField', () => {
  describe('maxButtonValueWei scenarios', () => {
    ...
    it('should use button value when wallet value is more than button value', async () => {
      ...
      await waitFor(() => {
        // Should use button value since it's smaller
        const expectedValueWei = buttonValue - MIN_REMAINING_BALANCE;
        const expectedValue = Number(expectedValueWei) / 1e18;
        expect(amountInput).toHaveValue(expectedValue);
      });
    });
    ...
    it('should disable max button when maxButtonValue is 0', () => {
      const verySmallButtonValue = MIN_REMAINING_BALANCE / 2n; // This will result in 0 after processing

      renderAmountField({
        maxWalletValueWei: walletBalance,
        maxButtonValueWei: verySmallButtonValue,
      });
    ...
    it('should display gas fee warning for CELO when balance is insufficient', () => {
